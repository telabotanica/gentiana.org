...
// |                                            CORPS du PROGRAMME                                        |
// +------------------------------------------------------------------------------------------------------+
class NaturalisteNomRelationSqlDao extends aDaoSqlEflore implements iDaoEnanr {
	/*** Attributes : ***/
	private $table_nom = 'eflore_naturaliste_nom_relation';
	private $table_prefixe = 'enanr_';
	protected $table_cle = array('nom_naturaliste_01', 'nom_naturaliste_02', 'categorie_naturaliste');
	private $table_champs = array(
		'enanr_id_nom_naturaliste_01'	=> array('type' => 'id', 'format' => 'int'),
		'enanr_id_nom_naturaliste_02'	=> array('type' => 'id', 'format' => 'int'),
		'enanr_id_categorie_naturaliste'	=> array('type' => 'id', 'format' => 'int'),
		'enav_id_valeur_naturaliste'	=> array('type' => 'no', 'format' => 'int'),
		'enanr_notes_relation_nom'	=> array('type' => 'no', 'format' => 'str')
		);
	...
	public function consulter($cmd, $parametres = array() )
	{
		switch($cmd) {
			case iDaoEnanr::CONSULTER :
				$sql = 	'SELECT * '.
						'FROM '.$this->getBddPrincipale().'.eflore_naturaliste_nom_relation';
				$this->setResultatType(aDaoSql::RESULTAT_OBJET_MULTIPLE);
				break;
			case iDaoEnanr::CONSULTER_ID :
				$sql = 	'SELECT * '.
						'FROM '.$this->getBddPrincipale().'.eflore_naturaliste_nom_relation '.
						'WHERE enanr_id_nom_naturaliste_01 = ? AND enanr_id_nom_naturaliste_02 = ? AND enanr_id_categorie_naturaliste = ? ';
				break;
			case iDaoEnanr::CONSULTER_ID_MAX :
				$sql = 	'SELECT MAX(enanr_id_nom_naturaliste_01) '.
						'FROM '.$this->getBddPrincipale().'.eflore_naturaliste_nom_relation '.
						'WHERE enanr_id_nom_naturaliste_02 = ? ';
		...
		$this->setRequete($sql);
		return parent::consulter($parametres, NaturalisteNomRelation::CLASSE_NOM);
	}
