...
// |                                            CORPS du PROGRAMME                                        |
// +------------------------------------------------------------------------------------------------------+
class NaturalisteComposerIntituleNomSqlDao extends aDaoSqlEflore implements iDaoEnacin {
	/*** Attributes : ***/
	private $table_nom = 'eflore_naturaliste_composer_intitule_nom';
	private $table_prefixe = 'enacin_';
	protected $table_cle = array('nom_naturaliste', 'intitule_nom_naturaliste');
	private $table_champs = array(
		'enacin_id_nom_naturaliste'	=> array('type' => 'id', 'format' => 'int'),
	...
	public function consulter($cmd, $parametres = array() )
	{
		switch($cmd) {
			case iDaoEnacin::CONSULTER :
				$sql = 	'SELECT * '.
						'FROM '.$this->getBddPrincipale().'.eflore_naturaliste_composer_intitule_nom';
				$this->setResultatType(aDaoSql::RESULTAT_OBJET_MULTIPLE);
				break;
			case iDaoEnacin::CONSULTER_ID :
				$sql = 	'SELECT * '.
						'FROM '.$this->getBddPrincipale().'.eflore_naturaliste_composer_intitule_nom '.
						'WHERE enacin_id_nom_naturaliste = ? AND enacin_id_intitule_nom_naturaliste = ? ';
				break;
			case iDaoEnacin::CONSULTER_ID_MAX :
				$sql = 	'SELECT MAX(enacin_id_nom_naturaliste) '.
						'FROM '.$this->getBddPrincipale().'.eflore_naturaliste_composer_intitule_nom '.
						'WHERE enacin_id_intitule_nom_naturaliste = ? ';
		...
		$this->setRequete($sql);
		return parent::consulter($parametres, NaturalisteComposerIntituleNom::CLASSE_NOM);
	}
